...
namespace dc {
  ...
  private:
    ...
    cv::Mat m_mat_reduce_pixels;
    cv::Mat m_mat_CCs;
    cv::Mat m_mat_contour;

    int m_inputType;

    int m_width, m_height;
    int m_avgBackground, m_avgForeground, m_nb_connectedcomponants, m_MAX_a0;
    // Kanungo parameters
    double m_alpha, m_beta, m_alpha0, m_beta0, m_nf, m_nb;
    // Grayscale Model parameters
    float m_a0, m_g, m_MAX_Gradient, m_AVG_Gradient, m_sigma_gaussian;
    bool m_is4connected;
    std::vector<Seedpoint> m_listSeedPoints;
    std::vector<Pixel> m_listPixels;

    std::vector<std::vector<cv::Point> > m_contours;
    // connected component
    //std::vector<CC*> _listCCs;
    float m_Mean_CC_Distance, m_Max_CC_Distance;
    float m_Mean_CC_Stroke, m_Max_CC_Stroke;
    float m_Total_Degradation;
    int m_nbSPs_User;
    int m_local_zone;
...
#include <chrono>
  ...
  GrayscaleCharsDegradationModel::GrayscaleCharsDegradationModel(
    ...
    , m_MAX_Gradient(0.0f)
    , m_AVG_Gradient(0.0f)
    , m_sigma_gaussian(0.0f)
    , m_is4connected(false)
    , m_Mean_CC_Distance(0.0f)
    , m_Max_CC_Distance(0.0f)
    , m_Mean_CC_Stroke(0.0f)
    , m_Max_CC_Stroke(0.0f)
    , m_Total_Degradation(0.0f)
    , m_nbSPs_User(0)
    , m_local_zone(0)
  {
    initialize(img);
  ...
  GrayscaleCharsDegradationModel::~GrayscaleCharsDegradationModel()
  {
    m_listSeedPoints.clear();

    m_mat_output.release();
    m_mat_gray.release();
    m_mat_binary.release();
    m_mat_CCs.release();
  }
  ...
  {
    m_MAX_Gradient = INT_MIN;
    m_AVG_Gradient = 0;

    m_MAX_a0 = 20;
    m_Mean_CC_Stroke = 0;
    m_Max_CC_Stroke = INT_MIN;

    m_Total_Degradation = 0.0;
    ...
    m_inputType = imgInput.type();
    if (imgInput.channels() == 4) {
      cvtColor(imgInput, m_mat_gray, cv::COLOR_BGRA2GRAY);
    }
    else if (imgInput.channels() == 3) {
      cvtColor(imgInput, m_mat_gray, cv::COLOR_BGR2GRAY);
    }
    else if (imgInput.channels() == 1) {
      m_mat_gray = imgInput.clone();
    }
    else {
      m_nb_connectedcomponants = NO_CC;
      m_mat_output = imgInput.clone(); //need same type as input
      assert(m_mat_output.type() == m_inputType);
      return;
    }
    assert(m_mat_gray.type() == CV_8UC1);


    m_width = m_mat_gray.cols;
    m_height = m_mat_gray.rows;

    m_mat_binary = binarize(); //binarize m_mat_gray
    //set m_nb_connectedcomponants to NO_CC if color-uniform image,
    // but CCs are not yet computed.

    if (m_nb_connectedcomponants != NO_CC) {

      m_mat_output = m_mat_gray.clone();
      ...
      std::cerr << "Time extractConnectedComponantInfos="
		...
		<< "ms for m_nb_connectedcomponants=" << m_nb_connectedcomponants
		<< "\n";
      std::cerr << "Time calculatePixelsDistanceTransform="
		<< std::chrono::duration<double, std::milli>(time2).count()
		<< "ms\n";
      std::cerr << "m_listPixels.size()=" << m_listPixels.size() << "\n";
#endif
    }
    else {
      m_mat_output = imgInput.clone(); //to have same type as input
    }
  }
  ...
  */
					       ...
					       float D)
  {
	...
	qDebug() << "m_nbSPs_User=" << m_nbSPs_User;
      */
#ifdef TIMING
      auto t0 = std::chrono::steady_clock::now();
#endif
      ...
      //qDebug() << "seed point classification: get type "<< m_MAX_Gradient << " " << m_listSeedPoints.size() ;
    ...
    assert(m_inputType == CV_8UC1 ||
	   m_inputType == CV_8UC3 ||
	   m_inputType == CV_8UC4);
    if (m_mat_output.type() != m_inputType) {
      if (m_inputType == CV_8UC4) {
	cv::Mat out;
	cvtColor(m_mat_output, out, cv::COLOR_GRAY2BGRA);
	m_mat_output = out;
      }
      else if (m_inputType == CV_8UC3) {
	cv::Mat out;
	cvtColor(m_mat_output, out, cv::COLOR_GRAY2BGR);
	m_mat_output = out;
      }
    }
    assert(m_mat_output.type() == m_inputType);
  ...
  /*
    Fill m_listPixels, m_MAX_Gradient, m_AVG_Gradient

  */
  void
  GrayscaleCharsDegradationModel::calculatePixelsDistanceTransform()
  {
    assert(m_mat_reduce_pixels.type() == CV_8UC1);
    assert(m_mat_reduce_pixels.rows == m_height &&
	   m_mat_reduce_pixels.cols == m_width);
    assert(m_mat_binary.type() == CV_8UC1);
    assert(m_mat_binary.rows == m_height && m_mat_binary.cols == m_width);

    m_listPixels.reserve((m_width * m_height) / 5); //arbitrary !
    //We should compute the number of white pixels in m_mat_reduce_pixels
    // to have a less arbitrary capacity.

    //B:TODO:OPTIM? better way to compute gradient image.

    const int height = m_mat_reduce_pixels.rows;
    const int width = m_mat_reduce_pixels.cols;

    //size_t nbPixs = 0;

    for (int y = 0; y < height; ++y) {
      const uchar *m = m_mat_reduce_pixels.ptr<uchar>(y);
      const uchar *b = m_mat_binary.ptr<uchar>(y);
	...
	if (m[x] == 255) {
	  ...
	  //pixel.distanceToEdge = calculateDistanceFromBord(pixel, m_is4connected);
	  ...
	  if (pixel.distanceToEdge > 0) {
	    ...
	    // for _Max_Gradient & m_AVG_Gradient computation.
		...
		pixel.pos.y < (height - 1)) {
	      ...
	      const float dx =
		((int)m_mat_gray.at<uchar>(pixel.pos.y, pixel.pos.x + 1) -
		 (int)m_mat_gray.at<uchar>(pixel.pos.y, pixel.pos.x - 1)) *
		0.5f;
	      const float dy =
		((int)m_mat_gray.at<uchar>(pixel.pos.y + 1, pixel.pos.x) -
		 (int)m_mat_gray.at<uchar>(pixel.pos.y - 1, pixel.pos.x)) *
		0.5f;
	    ...
	    // MAX gradient value
	    if (m_MAX_Gradient < pixel.gradient_value) {
	      m_MAX_Gradient = pixel.gradient_value;
	    }

	    // AVG gradient value
	    m_AVG_Gradient += pixel.gradient_value;

	    m_listPixels.push_back(pixel);
	  }
	}
      }
    }

    if (!m_listPixels.empty())
      m_AVG_Gradient = m_AVG_Gradient / m_listPixels.size();

    m_mat_reduce_pixels.release();
  }
  ...
  /*
    ...
    if (m_mat_binary.at<uchar>(p.y, p.x) != m_mat_binary.at<uchar>(p1.y, p1.x))
    ++type;
    }
  ...
  */
								...
								cv::Point &B) const
  {
    ...
    assert(m_mat_CCs.rows == m_height && m_mat_CCs.cols == m_width &&
	   m_mat_CCs.type() == CV_8UC1);

    int min_stroke_width_at_seed_point = 0;

    const int width = m_mat_CCs.cols;
    const int height = m_mat_CCs.rows;
    ...
    cv::Point A_0, B_0;
    {
      const uchar *m = m_mat_CCs.ptr<uchar>(yo);
    ...
    for (int y = yo; y < height; ++y) {
      assert(y >= 0 && y < m_mat_CCs.rows && xo >= 0 && xo < m_mat_CCs.cols);
      if (m_mat_CCs.at<uchar>(y, xo) == 0) {
	++d_90;
	A_90.x = xo;
	A_90.y = y;
      } else
	...
	break;
      ...
      assert(y >= 0 && y < m_mat_CCs.rows && x >= 0 && x < m_mat_CCs.cols);
      if (m_mat_CCs.at<uchar>(y, x) == 0) {
	++d_45;
	A_45.x = x;
	A_45.y = y;
      }
  ...
  */
  ...
  GrayscaleCharsDegradationModel::calculateNoiseRegionType()
  {
    ...
    const int localZone = m_local_zone;

    for (Seedpoint &sp : m_listSeedPoints) {
      ...
      const int bx = std::min(xo + localZone, m_width - 1);
      const int cy = std::min(yo + localZone, m_height - 1);
	...
	const float k = -tanf(sp.pixel.gradient_angle * static_cast<float>(CV_PI) / 180.0f);
	...
	listPointsInLine = getLinePoints(pB1, pB2, m_width, m_height);
	...
	const uchar vp = m_mat_CCs.at<uchar>(p.y, p.x);
	const uchar vp1 = m_mat_CCs.at<uchar>(p1.y, p1.x);
    ...
    //std::cerr<<"dbg1="<<dbg1<<" dbg2="<<dbg2<<" dbg3="<<dbg3<<" m_listSeedPoints.size()="<<m_listSeedPoints.size()<<"\n";
  }
  ...
  GrayscaleCharsDegradationModel::assignmentSizeOfNoiseRegion()
  {
	...
	if (sp.b_cheval > m_Mean_CC_Stroke)
	  delta_a = m_Mean_CC_Stroke - sp.b_tache;
	...
	if (sp.b_tache <= 3.f)
	  ...
	  randValue = 0.4f * m_Mean_CC_Stroke / sp.b_tache;
  ...
  /*
    ...
    Fill m_mat_reduce_pixels, m_mat_CCs, m_mat_contour && m_Mean_CC_Stroke.
    Update m_nb_connectedcomponants
  */
  void
  GrayscaleCharsDegradationModel::extractConnectedComponantInfos()
  {
    ...
    ConnectedComponent::extractAllConnectedComponents(
						      m_mat_binary, ccsInfo, connectivity);

    m_mat_reduce_pixels = cv::Mat::zeros(m_mat_binary.size(), m_mat_binary.type());
    m_mat_CCs = cv::Mat(m_mat_binary.size(), m_mat_binary.type());

    cv::Mat mat_Accepted_CCs =
      cv::Mat::zeros(m_mat_binary.size(), m_mat_binary.type());

    //set m_mat_CCs to all white (255).
    int rows = m_mat_CCs.rows;
    int cols = m_mat_CCs.cols;
    if (m_mat_CCs.isContinuous()) {
      cols *= rows;
      rows = 1;
    }
    for (int y = 0; y < rows; ++y) {
      uchar *m = m_mat_CCs.ptr<uchar>(y);
    ...
    int max_dis = std::max(m_width, m_height) / 35;
    if (m_width * m_height >= 1000000)
      max_dis = std::max(m_width, m_height) / 15;
	...
	const int max_disB = static_cast<int>((mean + stdDev) * 3.34);
	...
	We do not set an absolute maximum for now. Thus if we only have
      ...
      max_dis = static_cast<int>(std::min(*itMed * 2, size_t(mean+0.5)));
      //std::cerr<<"max_dis="<<max_dis<<"\n";
    }
#endif //0
	...
	if (dis_square <= max_dis_square) { //(to remove big CCs)
	  ...
	  for (int y = min_y; y <= max_y; ++y) {
	    const uchar *m = m_mat_binary.ptr<uchar>(y);
	  ...
	  for (int x = min_x; x <= max_x; ++x) {
	    for (int y = min_y; y <= max_y; ++y) {
	      ...
	      {
		int d = 0;
		while (y < m_height && m_mat_binary.at<uchar>(y, x) == 0) {
		  ++d;
		  ++y;
		}
		if (d > 0) {
	  ...
	  //which version is correct ??? [it should change only m_Mean_CC_Stroke]
	  {
	  ...
	  assert(counter > 0);
	  m_Mean_CC_Stroke += width_of_CC / static_cast<float>(counter);

	  ++id_cc;

	  //paint enlarged AABB in m_mat_reduce_pixels
	  const int y0 = std::max(min_y - cc_height / 2, 0);
	  const int y1 = std::min(max_y + cc_height / 2, m_height - 1);
	  const int x0 = std::max(min_x - cc_width / 2, 0);
	  const int x1 = std::min(max_x + cc_width / 2, m_width - 1);
	  for (int y = y0; y <= y1; ++y) {
	    uchar *m = m_mat_reduce_pixels.ptr<uchar>(y);
	    for (int x = x0; x <= x1; ++x) {
	      assert(x >= 0 && x < m_width && y >= 0 && y < m_height);
	      m[x] = 255;
	    }
    ...
    m_Mean_CC_Stroke = m_Mean_CC_Stroke / id_cc;
    m_nb_connectedcomponants = id_cc;
    ...
    m_mat_contour = cv::Mat::zeros(m_mat_binary.size(), m_mat_binary.type());

    for (const std::vector<cv::Point> &v : m_contours)
      for (const cv::Point &p : v) //(copy of Point)
	m_mat_contour.at<uchar>(p.y, p.x) = 255;
							    ...
							    float percent_diffusion)
  {
    ...
    /**/
    size_t nbDiffusion =
      static_cast<size_t>(percent_diffusion * m_nbSPs_User * 0.01f);
    size_t nbCheval = static_cast<size_t>(percent_cheval * m_nbSPs_User * 0.01f);
    assert(nbDiffusion + nbCheval <= (size_t)m_nbSPs_User);
    const size_t nbIndependent = m_nbSPs_User - nbDiffusion - nbCheval;

    std::vector<float> listFirstThreslhold;
    std::vector<float> listSecondThreslhold;
    listFirstThreslhold.reserve(m_nbSPs_User);  //B
    listSecondThreslhold.reserve(m_nbSPs_User); //B
    size_t counter = 0;
    // Only the black seed-point pixels will become disconnection spots

    for (const Seedpoint &sp : m_listSeedPoints) {
    ...
    if (nbIndependent > 0) {
      for (Seedpoint &sp : m_listSeedPoints) {
	if (sp.type == 3) {
	  if (sp.b_tache > m_MAX_a0)
        sp.b_tache = static_cast<float>(m_MAX_a0);
    ...
    for (size_t i = 0; i<m_listSeedPoints.size(); ++i) {
      const Seedpoint &sp = m_listSeedPoints[i];
	...
	assert(size_t(removeIds[i]) < m_listSeedPoints.size());
	m_listSeedPoints.erase(m_listSeedPoints.begin()+removeIds.at(i));
      }

#else

    const size_t sz0 = m_listSeedPoints.size();
	...
	std::swap(m_listSeedPoints[i], m_listSeedPoints[sz]);
      }
      else {
	++i;
      }
  ...
  /*
    ...
    listRecs.reserve(m_listSeedPoints.size());

    // calculate mean by region
    int meanB = m_avgBackground;
    int meanF = m_avgForeground;

    MyRNG rng((unsigned int) time(nullptr));
    // generator for background
    NormalDistribution var_nor_Background(rng, meanB, m_sigma_gaussian);
    // generator for foreground
    NormalDistribution var_nor_Forground(rng, meanF, m_sigma_gaussian);
    ...
    int semi_minor_axis = floorf((1 - m_g)*sp.size + 0.5);
    ...
    if (std::max(abs(C_gris - min_B), abs(C_gris - max_B))>m_MAX_Gradient && C_gris>(m_avgBackground - 15))
    C_gris = m_avgBackground - 15;
    ...
    m_mat_output.at<uchar>(sp.pixel.pos.y, sp.pixel.pos.x) = meanB;
    else{
    for ( float* line : lines) {

    RapidlyDegradeLine(line, sp.pixel.pos, C_gris, m_mat_output);
    }
    for ( float* line : lines)
    free(line);
    lines.clear();
    ...
    // The difference between generated number and MEAN is not upper than SIGMA.
    if (std::max(abs(C_gris - min_B), abs(C_gris - max_B))>m_MAX_Gradient)
    C_gris = m_avgForeground;
    ...
    if (lines.size()<=1 && (lines.at(0))[0] < 4)
    m_mat_output.at<uchar>(sp.pixel.pos.y, sp.pixel.pos.x) = meanF;
    else{
    ...
    if (r.x>=0 && r.y>= 0 && (r.x + r.width)>=0 && (r.x + r.width) <m_width
    && (r.y+ r.height)>=0 && (r.y+ r.height) <m_height
    && r.width >= mask && r.height>=mask) {
    cv::Mat roiImg = m_mat_output(r);
    cv::Mat roiImg_Blur;
    cv::GaussianBlur(roiImg, roiImg_Blur, cv::Size(mask,mask), 0);

    for (int x=0;x<r.width;++x)
    for (int y = 0;y<r.height;++y)
    if ((r.y+y)<m_height && (r.y + y) >=0 && (r.x + x) < m_width && (r.x + x)>=0)
    m_mat_output.at<uchar>(r.y + y, r.x +x) = roiImg_Blur.at<uchar>(y, x);
  ...
  */
  ...
  GrayscaleCharsDegradationModel::grayscaleDegradationByTypes()
  {
    ...
    for (const Seedpoint &sp : m_listSeedPoints) {
      ...
      // generator for foreground
      float mean_g = 15.f; //(1 - 1/(double)sp.size);
      float sigma_g = 0.15f;
	...
	sigma_g = 2.5f;
      ...
      m_g = var_nor_g();
      int limite_g = 0;
      //while (m_g<=(mean_g-sigma_g) || m_g>=(mean_g+sigma_g || m_g <0 || m_g>1) ) {
      while (m_g <= (mean_g - sigma_g) || m_g >= (mean_g + sigma_g) || m_g < 0 ||
	     m_g > 1) { //B ???
	m_g = var_nor_g();
	++limite_g;
	if (limite_g > 500) {
      ...
      if (m_g > 0.9f)
	m_g = 0.9f;
      else if (m_g < 0.1f && sp.pixel.isBackground)
	m_g = 0.1f;
      if (m_g < 0.f)
	m_g = mean_g;
	...
	semi_major_axis = 3;
#endif
      const int semi_minor_axis = static_cast<int>(floorf((1 - m_g) * sp.size + 0.5f));
	...
	if (lines.size() == 1 && (lines[0])[0] < 4) {
	  ...
	  for (const float *line : lines) {
	    DegradedLine(
			 line, sp.pixel.pos, C_gris, 10, m_sigma_gaussian, m_mat_output, true);
	  }
	}
      }
      // background to foreground
	...
	if (lines.size() <= 1 && (lines.at(0))[0] < 4) {
	  ...
	  for (const float *line : lines) {
	    DegradedLine(
			 line, sp.pixel.pos, C_gris, 10, m_sigma_gaussian, m_mat_output, false);
	  }
	}
      }
    ...
    for (const cv::Rect &r : listRecs) {
      if (r.x >= 0 && r.y >= 0 && (r.x + r.width) < m_width &&
	  (r.y + r.height) < m_height && r.width >= mask && r.height >= mask) {
	...
	//copy roiImg in m_mat_output
	for (int y = 0; y < r.height; ++y) {
	  uchar *dst = m_mat_output.ptr<uchar>(r.y + y);
	  const uchar *src = roiImg_Blur.ptr<uchar>(y);
	  for (int x = 0; x < r.width; ++x) {
	    assert((r.y + y) < m_height && (r.y + y) >= 0 && (r.x + x) < m_width &&
		   (r.x + x) >= 0);
	    dst[r.x + x] = src[x];
	  }
	}
					       ...
					       bool isFtoB)
  {
	...
	if (isFtoB) {
	  ...
	  B1_moyen_gris = /*C_gris + (B_gris - C_gris)*dis/deltaDis;}*/ expf(
									    (logf(B_gris) - logf(Centre_gray)) * (dis / deltaDis) +
									    logf(Centre_gray));
  ...
  /**
     Compute minimal distance from @a pixel to the closest white contour in m_mat_contour,
     in eight directions.
  ...
  */
									 ...
									 const Pixel &pixel) const
  {
    ...
    if (m_mat_contour.at<uchar>(y, x) == WHITE) //is edge pixel
      return 0;

    int minDist = 0;

    int dist = 0;
    const int x0 = std::max(x - m_local_zone, 0);
    {
      const uchar *mc = m_mat_contour.ptr<uchar>(y);
      for (int i = x; i >= x0; --i) {
	assert(i >= 0);
	if (mc[i] == WHITE) {
	  break;
    ...
    dist = 0;
    const int x1 = std::min(x + m_local_zone, m_width);
    {
      assert(y < m_mat_contour.rows);
      const uchar *mc = m_mat_contour.ptr<uchar>(y);
      for (int i = x; i < x1; ++i) {
	assert(i < m_width);
	if (mc[i] == WHITE) {
    ...
    dist = 0;
    const int y0 = std::max(y - m_local_zone, 0);
    for (int i = y; i >= y0; --i) {
      assert(i >= 0);
      assert(i >= 0 && i < m_mat_contour.rows && x >= 0 && x < m_mat_contour.cols);
      if (m_mat_contour.at<uchar>(i, x) == WHITE) {
	break;
      }
      ++dist; // 90 degree
      if (dist >= minDist) {
    ...
    dist = 0;
    const int y1 = std::min(y + m_local_zone, m_height);
    for (int i = y; i < y1; ++i) {
      assert(i < m_height);
      assert(i >= 0 && i < m_mat_contour.rows && x >= 0 && x < m_mat_contour.cols);
      if (m_mat_contour.at<uchar>(i, x) == WHITE) {
    ...
    for (int i = x; i < x1; ++i) {
      ...
      if (j >= y0) {
	assert(i < m_width);
	assert(j >= 0 && j < m_mat_contour.rows && i >= 0 &&
	       i < m_mat_contour.cols);
	if (m_mat_contour.at<uchar>(j, i) == WHITE) {
	  break;
	}
	++dist; //45 degree
	if (dist >= minDist) {
							    ...
							    const Pixel &pixel) const
  {
    ...
    const int localZone = rate * m_local_zone;
    const int y0 = std::max(pixel.pos.y - localZone, 0);
    const int y1 = std::min(pixel.pos.y + localZone, m_height);
    const int x0 = std::max(pixel.pos.x - localZone, 0);
    const int x1 = std::min(pixel.pos.x + localZone, m_width);

    float minDistSq = FLT_MAX;

    for (int y = y0; y < y1; ++y) {
      const uchar *m = m_mat_contour.ptr<uchar>(y);
      for (int x = x0; x < x1; ++x) {
						   ...
						   double theta)
  {
    ...
    cv::Mat box = cv::Mat::zeros(cv::Size(m_width + ex, m_height + ex),
				 CV_8UC1); //B: TOO BIG ??????
    ...
    const int localZone2 = 2 * m_local_zone;
    const int xr =
	...
	const int y1 = std::min(std::min(max_y + 5, box.rows), m_mat_gray.rows + ex / 2);
	const int x0 = std::max(std::max(min_x - 5, 0), ex / 2);
	const int x1 = std::min(std::min(max_x + 5, box.cols), m_mat_gray.cols + ex / 2);
	for (int y = y0; y < y1; ++y) {
	  const uchar *b = box.ptr<uchar>(y);
	  const int yt = y - ex / 2;
	  assert(yt >= 0 && yt < m_mat_gray.rows);
...
#if 0  //#ifndef NDEBUG
	if (! (pt.y >= 0 && pt.y < m_mat_gray.rows 
	       && pt.x >= 0 && pt.x < m_mat_gray.cols)) {
	  std::cerr<<"pt.x="<<pt.x<<" m_mat_gray.cols="<<m_mat_gray.cols<<"\n";
	  std::cerr<<"pt.y="<<pt.y<<" m_mat_gray.rows="<<m_mat_gray.rows<<"\n";
	}
#endif //NDEBUG

	if (pt.y >= m_mat_gray.rows)
	  pt.y = m_mat_gray.rows - 1;
	else if (pt.y < 0)
	  pt.y = 0;
	if (pt.x >= m_mat_gray.cols)
	  pt.x = m_mat_gray.cols - 1;
	else if (pt.x < 0)
	  pt.x = 0;

	assert(pt.y >= 0 && pt.y < m_mat_gray.rows && pt.x >= 0 &&
	       pt.x < m_mat_gray.cols);
	const int gray = m_mat_gray.at<uchar>(pt.y, pt.x);
	...
	if (line != nullptr) {
	  ...
	  for (const cv::Point &px : pointInLine) {
	    ...
	    assert(px.y >= 0 && px.y < m_mat_gray.rows && px.x >= 0 &&
		   px.x < m_mat_gray.cols); //B
  ...
  */
  ...
  /*
    @return binarized version of m_mat_gray
    Will also update m_avgBackground & m_avgForeground
    with black pixels (in binarized image) considered as foregound
    and the rest considered as background.

    If no foreground pixel is found, set m_nb_connectedcomponants to NO_CC.
  */
  cv::Mat
  GrayscaleCharsDegradationModel::binarize()
  {
    cv::Mat img_bin;
    cv::threshold(m_mat_gray, img_bin, 0, 255, cv::THRESH_OTSU);
    ...
    if (img_bin.isContinuous() && m_mat_gray.isContinuous()) {
      cols *= rows;
      rows = 1;
    }
    for (int i = 0; i < rows; ++i) {
      const uchar *m = m_mat_gray.ptr<uchar>(i);
  ...
  /*
    Update pixels, in m_listPixels, probability according to their distanceToEdge & isBackground.
  ...
  */
  ...
  GrayscaleCharsDegradationModel::calculatePixelsProbability()
  {
	...
	pixel.probability = m_beta0 * static_cast<float>(exp((-m_beta) * d * d)) + m_nf;
	...
	pixel.probability = m_alpha0 * static_cast<float>(exp((-m_alpha) * d * d)) + m_nb;
      }
    }
  }
  ...
  {
    const size_t sz = m_listPixels.size();
    if (sz == 0) {
      return;
    }
    ...
    for (const Pixel &pixel : m_listPixels) {
      listProbability.push_back(pixel.probability);
    }
    assert(m_listPixels.size() == listProbability.size());

    std::sort(listProbability.begin(), listProbability.end());

    const size_t nbSps_Over =
      static_cast<size_t>(m_nbSPs_User + m_nbSPs_User * percent_independent / 100.f);
    ...
    m_listSeedPoints.reserve(
			    std::max(sz / 100, static_cast<size_t>(m_height))); //arbitrary
	...
	//++countBF;
	m_listSeedPoints.emplace_back(pixel);
      }
    }
  ...
  namespace GrayscaleCharsDegradation {
    ...
    cv::Mat degradation(const cv::Mat &img, int level, float indepentSpotsRatio, float overlappingSpotsRatio)
    {
      ...
      return cdm.degradate_cv(level, percentOfIndepentSpots, percentOfOverlappingSpots, percentageOfDisconnectionSpots);
    }
